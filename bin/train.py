...
def safe_load(filepath):
    """Load a PyTorch file with proper handling for different PyTorch versions"""
    try:
        # First try with weights_only=False (for PyTorch 2.6+)
        return torch.load(filepath, weights_only=False)
    except TypeError:
        # Fall back to default for older PyTorch versions
...
def main():
    ...
    # Load datasets with version-safe loader
    train_data = safe_load(args.train_data)
    val_data = safe_load(args.val_data)
